...
def sample_from_model(key, t_mean, t_std, params, N):
    ...
    direction = normalize(jnp.ones(D))
...
def initialize_model(X, K):
    ...
    log_sigmas_plane = jnp.log(jnp.std(jnp.concatenate(residuals), axis=0))
...
def main():
    ...
    direction = jnp.ones(X_train.shape[1])
    direction = direction / jnp.linalg.norm(direction)
    params_init = initialize_model(X_train, 2)
    t = project_data(X_train, 0, direction)
